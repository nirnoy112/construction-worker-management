          		...
          		<div class="row clearfix">
					...
					<div class="col-sm-12">
	          		...
	          		<div class="col-sm-12">
	          			...
	          			<div class="col-sm-6" style="border: 4px solid; height: 280px;">
...
<script>
	...
	Microblink.SDK.RegisterListener({
	  onScanSuccess: (data) => {
	    ...
	    let results = data.result.data[0].result;
	    if (results == null) {
	      $('#scannerBody').html('<p>Scanning is finished, but we could not extract the datafrom Microblink API.</p><p>Please check if you uploaded the right document type.</p>');
	    	...
	    	$('#identificationId').val(results.customerIdNumber);
	    	$('#firstName').val(results.customerFirstName);
	    	$('#lastName').val(results.customerFamilyName);
	    	$('#middleName').val(results.customerMiddleName);
	    	if(results.sex == 1) {
	    	...
	    	$('#dob').val(results.dateOfBirth.originalString.substring(0, 2) + '-' + results.dateOfBirth.originalString.substring(2, 4) + '-' +results.dateOfBirth.originalString.substring(4));
	    	$('#address1').val(results.addressStreet);
	    	$('#city').val(results.addressCity);
	    	$('#zipCode').val(results.addressPostalCode);
	    	$('#state').val(results.addressJurisdictionCode);
	    	var sex = $('input[type=radio][name=sex]').val();
	    	console.log(sex);
	      ...
	      $('#scannerBody').html(
	      	...
	      	'<br>Identification ID:   ' + results.customerIdNumber + 
	      	'<br>First Name:          ' + results.customerFirstName + 
	      	'<br>Last Name:           ' + results.customerFamilyName + 
	      	'<br>Middle Name:         ' + results.customerMiddleName + 
	      	'<br>Sex :                ' + ((results.sex == 2) ? 'Male' : 'Female') +
	      	'<br>Date Of Birth :      ' + (results.dateOfBirth.originalString.substring(0, 2) + '-' + results.dateOfBirth.originalString.substring(2, 4) + '-' +results.dateOfBirth.originalString.substring(4)) + 
	      	'<br>Address :            ' + results.addressStreet + 
	      	'<br>City:                ' + results.addressCity + 
	      	'<br>Zip Code:            ' + results.addressPostalCode + 
	      	'<br>State:               ' + results.addressJurisdictionCode
	      	);
	    }
