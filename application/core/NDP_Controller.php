	...
	class NDP_Controller extends CI_Controller {
		...
		public function loadAppData($is_authorization_required) {
			...
			if($is_authorization_required == true) {
				...
				$userData = $this->User_model->get_user($data['id']);

				$data['clocked_in'] = $userData['clocked_in'];
				$data['breaked_in'] = $userData['breaked_in'];
				$data['tc_site_id'] = $userData['tc_site_id'];
				$data['tc_company_id'] = $userData['tc_company_id'];
				$data['tc_site'] = $userData['tc_site'];
				$data['tc_company'] = $userData['tc_company'];
...
$config['smtp_user'] = 'portal4sbp@gmail.com';
$config['smtp_pass'] = 'pass4portalsbp';
...
$this->email->set_newline("\r\n");
        	...
        	$this->email->from('no_reply@ohstrainconsult.com', 'OHS Training & Consulting, INC.', 'no_reply@ohstrainconsult.com');
        	$this->email->to($toEmail);
        	$this->email->subject($subject);
        	$this->email->message($message);
			//$this->email->set_newline("\r\n");
